...
5. Follow the applicable persistence mode in `references/conventions/project-profile.md` when this Implementation establishes a greenfield project's durable structure, initializes a missing established-project handoff from durable conventions already established by the current bootstrap, or implements an explicitly requested project-wide redesign/migration that changes durable structural conventions. Established-project handoff initialization must not expand discovery or persist task-local observations merely to populate the profile. Do not otherwise persist ordinary task-local feature work or restructuring merely because this skill was used.
