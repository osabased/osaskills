...
7. With Team Create or multiple local editors, account for overlapping collaborators or sync processes. Avoid simultaneous editing of the same synced script because Roblox documents that collaborators can overwrite one another. Packages can be synced, but `PackageLink` itself is not written to disk.
