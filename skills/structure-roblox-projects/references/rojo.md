...
1. Inspect the repository's Rojo pin and run `rojo --version` when available.
...
4. Trace the full DataModel effect of a mapping change. A project or metadata edit can affect instances outside the source subtree that appears to contain the change.
5. Keep structural migration separate from Rojo version upgrades. Change pins, manifests, lockfiles, or generated mappings only when those writes belong to the authorized task.
