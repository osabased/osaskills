...
**Direct generated-child use.** When the child's reconciliation policy is `required`, load matching resource-bound learnings alongside schema-version 3 resource records before applying version-sensitive guidance. When its policy is `conditional`, do not load records or learnings on the healthy declared-pin plus lock/header path; load them only after a listed state escalation trigger activates full reconciliation. A soft instruction defect activates parent repair diagnosis without automatically loading lifecycle state; load it only if diagnosis finds a hard/state trigger or an authorized repair needs lifecycle evidence. A `not-applicable` child does not consume lifecycle state for ordinary use. In every policy that loads learnings, a learning directs a current check rather than deciding it. A current matching block or an adverse observation that remains material after re-checking stops the affected use and activates `roblox-resource-acquisition` in `repair/reconcile` mode.
