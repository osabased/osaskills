...
A scripted follow-up supplies new user evidence; it does not prove that the agent elicited that evidence well. Judge each turn using only what was available then. A later correction does not erase an earlier invented preference, unauthorized commitment, or failure to offer a useful discovery step. An initially correct hypothesis need not change just to demonstrate revision.
...
2. Baseline `skills/preference-discovery/SKILL.md` at `e5100ad15766e255c948bf4ad2908d62d8aa8772`.
...
The original boundary cases remain unchanged. These additions exercise feedback over an exchange and pair useful discovery with checks against unnecessary discovery:

| Cases | Question tested |
|---|---|
| `closer-not-ready` / `accept-and-continue` | Does relative feedback guide refinement without premature approval, and does explicit acceptance lead to the requested work without another interview? |
| `tone-diagnosis-correction` | Does the agent act on the property the user identifies, rather than keep varying a dimension the user says does not matter? |
| `contextual-interruptions` / `clear-rule-no-exceptions` | Can it identify relevant conditions without inventing exceptions or reopening a clear local rule? |
| `combined-experience-correction` / `unchanged-combination-reuse` | Can it investigate a problematic combination while preserving successful choices, without requiring a new test for an unchanged, already-approved combination? |
...
For conversational cases, also judge whether the agent offers an informative next step, uses the subsequent feedback to reach a supported local decision, and carries that decision into the requested draft or next action. Track avoidable questions, repeated explanations, and samples that do not address the remaining uncertainty. A guess does not win because it uses fewer turns; endless caution does not pass because it avoids commitment. Distinguish a useful provisional sample from an unsupported final choice. Where tools or authorization limit completion, a concrete supported next action is enough; do not demand an unavailable artifact.

Use paired safeguards: `indifferent-format` with `unknown-not-consent`, `authorized-code-probe` with `restricted-experiment`, and `local-preference-scope` with `explicit-broad-scope`, as well as the pairs above. Changes should not fix one side by breaking the other. Include the UI and motion cases when testing generalized coverage so that non-UI gains do not conceal UI regressions.
...
For evaluating the earlier generalization, the historical baseline is `skills/ui-direction-discovery/SKILL.md` at `3060aae885ed60c54e3b41ad62eac1e7025333f6`. Its UI-only scope is a known limitation, not an expectation to hide when reporting broadened coverage. Use the current-skill baseline above to judge this revision rather than attributing earlier improvements to it.

The historical path above is intentional. For a full checkout, search active references after a rename:
